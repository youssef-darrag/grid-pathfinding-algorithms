...
class AStarSearch:
    def __init__(self, grid: Grid):
        ...
        Args:
            grid: Grid object containing environment
        """
        self.grid = grid
    ...
    def search(self) -> Tuple[Optional[List[Tuple[int, int]]], Dict]:
        ...
        Returns:
            Tuple containing:
            - Path as list of coordinates (or None if no path)
        ...
        start_node = Node(
            ...
            parent=None,
            cost=0,  # g(n)
            heuristic=self.grid.manhattan_distance(start, goal)  # h(n)
        )
        
        # Add start node to frontier
        heapq.heappush(frontier, (start_node.total_cost, start_node))
        
        # Track best g_scores for each position
        g_scores = {start: 0}
        
        # Track nodes in memory
        ...
        while frontier:
            ...
            for neighbor_pos in self.grid.get_neighbors(current_pos):
                ...
                if neighbor_pos not in g_scores or tentative_g_score < g_scores[neighbor_pos]:
                    ...
                    h_score = self.grid.manhattan_distance(neighbor_pos, goal)
...
def test_astar():
    ...
    print(f"Manhattan distance: {grid.manhattan_distance(grid.start, grid.goal)}")
